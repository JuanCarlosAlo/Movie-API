...
const fillSliderItem = (arrayObj, data) => {
...
const createSlider = async (mainContainer, arrayObj, allData) => {
  ...
  for (let index = 0; index < numberOfSliderItems; index++) {
    const sliderItem = fillSliderItem(arrayObj, allData.results[index]);
...
const trendTab = (mainContainer, arrayObj, allData) => {
  createSlider(mainContainer, arrayObj, allData);
};
